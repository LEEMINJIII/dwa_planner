...
void DWAPlanner::create_obs_list(const sensor_msgs::LaserScan &scan)
{
  ...
  for (size_t i = 0; i < scan.ranges.size(); i++)
  {
    ...
    // laser frame에서의 좌표
    double x_laser = r * cos(angle);
    double y_laser = r * sin(angle);

    // odom frame으로 변환
    tf::Vector3 point_laser(x_laser, y_laser, 0.0);
    tf::Vector3 point_odom = transform * point_laser;

    geometry_msgs::Pose p;
    p.position.x = point_odom.x();
    p.position.y = point_odom.y();
    p.position.z = 0.0;

    obs_list_.poses.push_back(p);
  }

  ROS_INFO_STREAM_THROTTLE(2.0, "[DWA] Updated obstacle list from LaserScan, count: " << obs_list_.poses.size());
}
